...
import { RightAngleFactory } from './RightAngleFactory';
...
export interface RightAngleLinkProps {
	...
	width?: number;
	smooth?: boolean;
	link: RightAngleModel;
	diagramEngine: DiagramEngine;
	factory: RightAngleFactory;
}
...
export class RightAngleWidget extends React.Component<RightAngleLinkProps, RightAngleLinkState> {
    ...
    public customGenerateLinePath(firstPoint: PointModel, lastPoint: PointModel): string {
	...
	render() {
		...
		// For loop just for first and last path
            ...
            if (points.length === 4) {
                let middleY = (points[2].getY() - points[1].getY()) / 2 + points[1].getY();
                points[1].setPosition(points[0].getX(), middleY);
                points[2].setPosition(points[3].getX(), middleY);
            }
		}
		...
		for (let j = 0; j < points.length - 1; j++) {
			paths.push(
				this.generateLink(
					this.customGenerateLinePath(points[j], points[j + 1]),
					{
